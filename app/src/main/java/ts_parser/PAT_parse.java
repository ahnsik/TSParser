...
public class PAT_parse {
    ...
    public PAT_parse(byte[] packetBuffer) {
        ...
        for (int i=0; i<count; i++) {
            _program_num[i] = (short) ((packetBuffer[offset]<<8)|packetBuffer[offset+1]);
            _PMT_PID[i] = (short) (((packetBuffer[offset+2]<<8)|packetBuffer[offset+3])&0x1FFF);
            offset += 4;
        }
